...
try {
  ...
  message.channel.send(error(`Oops! Seems like that command didn't work as expected! Please contact r3d is my dad#1001 to fix this using the error information below. \`\`\`xl\n${err}\`\`\``))
  ...
  if (!message.content.startsWith(prefix)){
    ...
    xpSchema.findOne({ guildID : message.guild.id, userID: message.author.id }, async (err, data) => {
      ...
      if (err) return console.log(`${magenta('[Karuma-Promise ERROR]')} : Unable to connect to MongoDB.`)
      ...
      if (!data) return console.log(`${magenta('[Karuma-Promise ERROR]')} : Failed to create new server xp document for ${message.guild.name}.`)
