...
module.exports = (client, member) => {
  ...
  if (!channel) return console.log(`${magenta('[Karuma-Promise ERROR]')} : Goodbye Message is enabled, but no channel is found. [${member.guild.name}]`)

  if (!channel.permissionsFor(channel.guild.me).has('SEND_MESSAGES')) return console.log(`${magenta('[Karuma-Promise ERROR]')} : MISSING SEND_MESSAGES PERMISSIONS. Cannot send goodbye message [${member.guild.name}]`)
