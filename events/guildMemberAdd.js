...
module.exports = (client, member) => {
  ...
  if (!client.guildsettings.get(member.guild.id) || !client.guildsettings.get(member.guild.id).welcomeChannel){
    return  console.log(`${magenta('[karuma-Promise ERROR]')} : Welcome Message is disabled on ${member.guild.name}.`)
  }
  ...
  if (!channel) return console.log(`${magenta('[karuma-Promise ERROR]')} : Welcome Message is enabled, but no channel is found. [${member.guild.name}]`)

  if (!channel.permissionsFor(channel.guild.me).has('SEND_MESSAGES')) return console.log(`${magenta('[karuma-Promise ERROR]')} : MISSING SEND_MESSAGES PERMISSIONS. Cannot send welcome message [${member.guild.name}]`)
